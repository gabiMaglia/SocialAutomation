...
const PostPreview = ({ post, compactMode = false, onClick }: PostPreviewProps) => {
  ...
  const CustomParagraph = React.forwardRef<HTMLParagraphElement, React.ComponentPropsWithoutRef<'p'>>(
    ({ children, ...props }, ref) => {
  ...
  const CustomStrong = React.forwardRef<HTMLElement, React.ComponentPropsWithoutRef<'strong'>>(
      ...
      >
        ...
        <Box p="sm" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <Group gap="xs" c="dimmed" mt="auto" style={{ fontSize: rem(12) }}>
            <Text span>{post.date ? new Date(post.date).toLocaleDateString() : ''}</Text>
          </Group>
        </Box>
      </Card>
    );
        ...
        <Box style={{ fontSize: rem(14), lineHeight: 1.5 }}>
          <ReactMarkdown
            ...
            components={{
              p: CustomParagraph,
              a: ({ node, ...props }) => (
                <Anchor {...props} target="_blank" rel="noopener noreferrer" c="#0a66c2" underline="hover" />
              ),
